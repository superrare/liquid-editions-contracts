...
contract DiagnoseLiquidSystem is Script {
    ...
    function _warnMissing(string memory label, address target) private {
        if (target == address(0)) {
            string memory msg_ = string.concat("WARN: ", label, " address is not set (component not deployed or not configured)");
            console.log(msg_);
            _recordIssue(msg_);
        }
    }
    ...
    function _factory(address target) private {
        ...
        try f.owner() returns (address value) { _emit("owner", value); }
        catch { console.log("owner: <error>"); }

        try f.paused() returns (bool value) {
            _emitBool("paused", value);
            _diagIf(value, "Factory is PAUSED - all token creation blocked");
        } catch { console.log("paused: <error>"); }

        try f.poolHooks() returns (address value) {
            _emit("poolHooks", value);
            _diagIf(value == address(0), "Factory.poolHooks is address(0) - Instant/MultiCurve creation will revert PoolHooksNotSet");
        } catch { console.log("poolHooks: <error>"); }

        try f.liquidRegistry() returns (address value) {
            _emit("liquidRegistry", value);
            _diagIf(value == address(0), "Factory.liquidRegistry is address(0) - tokens will not be registered after creation");
        } catch { console.log("liquidRegistry: <error>"); }

        try f.migrationExecutor() returns (address value) {
            _emit("migrationExecutor", value);
            _diagIf(value == address(0), "Factory.migrationExecutor is address(0) - migrateLiquidity will revert on all tokens");
        } catch { console.log("migrationExecutor: <error>"); }

        try f.poolManager() returns (address value) { _emit("poolManager", value); }
        catch { console.log("poolManager: <error>"); }
        ...
        } catch { console.log("baseToken: <error>"); }

        try f.protocolFeeRecipient() returns (address value) {
            _emit("protocolFeeRecipient", value);
            _diagIf(value == address(0), "Factory.protocolFeeRecipient is address(0) - graduated auction creation will revert");
        } catch { console.log("protocolFeeRecipient: <error>"); }

        try f.ccaFactory() returns (address value) { _emit("ccaFactory", value); }
        catch { console.log("ccaFactory: <error>"); }

        try f.lbpStrategyFactory() returns (address value) { _emit("lbpStrategyFactory", value); }
        catch { console.log("lbpStrategyFactory: <error>"); }

        try f.liquidInstantImplementation() returns (address value) {
            _emit("liquidInstantImplementation", value);
            _diagIf(value == address(0), "Factory.liquidInstantImplementation not set - createLiquidTokenInstant will revert");
        } catch { console.log("liquidInstantImplementation: <error>"); }

        try f.liquidMultiCurveImplementation() returns (address value) {
            _emit("liquidMultiCurveImplementation", value);
            _diagIf(value == address(0), "Factory.liquidMultiCurveImplementation not set - createLiquidTokenMultiCurve will revert");
        } catch { console.log("liquidMultiCurveImplementation: <error>"); }

        try f.liquidGraduatedImplementation() returns (address value) {
            _emit("liquidGraduatedImplementation", value);
        } catch { console.log("liquidGraduatedImplementation: <error>"); }

        try f.minRareLiquidityWei() returns (uint256 value) {
            console.log("minRareLiquidityWei");
            console.logUint(value);
        } catch { console.log("minRareLiquidityWei: <error>"); }

        try f.lpTickLower() returns (int24 value) { _emitInt24("lpTickLower", value); }
        catch { console.log("lpTickLower: <error>"); }

        try f.lpTickUpper() returns (int24 value) { _emitInt24("lpTickUpper", value); }
        catch { console.log("lpTickUpper: <error>"); }

        try f.poolTickSpacing() returns (int24 value) { _emitInt24("poolTickSpacing", value); }
        catch { console.log("poolTickSpacing: <error>"); }
    }
    ...
    function _router(address target, address rare, address usdc) private {
        ...
        try r.owner() returns (address value) { _emit("owner", value); }
        catch { console.log("owner: <error>"); }

        try r.paused() returns (bool value) { _emitBool("paused", value); }
        catch { console.log("paused: <error>"); }

        try r.liquidRegistry() returns (address value) { _emit("liquidRegistry", value); }
        catch { console.log("liquidRegistry: <error>"); }

        try r.universalRouter() returns (address value) { _emit("universalRouter", value); }
        catch { console.log("universalRouter: <error>"); }
        ...
        if (rare != address(0)) {
            ...
            } catch { console.log("isCurrencyWhitelisted(rare): <error>"); }
        }
        ...
        if (usdc != address(0)) {
            ...
            } catch { console.log("isCurrencyWhitelisted(usdc): <error>"); }
        }
    }
    ...
    function _guard(address target) private {
        ...
        try g.owner() returns (address value) { _emit("owner", value); }
        catch { console.log("owner: <error>"); }

        try g.factory() returns (address value) {
            _emit("factory", value);
            _diagIf(value == address(0), "LiquidGuard.factory is address(0) - Factory cannot call addInitializer, token creation will fail");
        } catch { console.log("factory: <error>"); }

        try g.feeDistributor() returns (address value) {
            _emit("feeDistributor", value);
            _diagIf(value == address(0), "LiquidGuard.feeDistributor is address(0) - fees silently skipped on every swap");
        } catch { console.log("feeDistributor: <error>"); }
        ...
        } catch { console.log("totalFeeBPS: <error>"); }
    }
    ...
    function _feeDistributor(address target, address guard, address registry) private {
        ...
        try d.owner() returns (address value) { _emit("owner", value); }
        catch { console.log("owner: <error>"); }

        try d.PROTOCOL_FEE_RECIPIENT() returns (address value) { _emit("protocolFeeRecipient", value); }
        catch { console.log("protocolFeeRecipient: <error>"); }
        ...
        try d.beneficiaryRegistry() returns (address value) {
            ...
            if (registry != address(0)) {
                ...
                _diagIf(!match_, "FeeDistributor.beneficiaryRegistry != LiquidRegistry (beneficiary payouts will be wrong)");
                _okIf(match_, "FeeDistributor.beneficiaryRegistry == LiquidRegistry");
            }
        } catch { console.log("beneficiaryRegistry: <error>"); }

        try d.conversionEnabled() returns (bool value) { _emitBool("conversionEnabled", value); }
        catch { console.log("conversionEnabled: <error>"); }

        try d.maxSlippageBps() returns (uint16 value) {
            console.log("maxSlippageBps");
            console.logUint(value);
        } catch { console.log("maxSlippageBps: <error>"); }

        try d.beneficiaryShareBPS() returns (uint16 value) {
            console.log("beneficiaryShareBPS");
            console.logUint(value);
        } catch { console.log("beneficiaryShareBPS: <error>"); }

        try d.totalFeeBPS() returns (uint16 value) {
            console.log("totalFeeBPS(immutable)");
            console.logUint(value);
        } catch { console.log("totalFeeBPS(immutable): <error>"); }

        try d.rareEthPoolKey() returns (
            address c0, address c1, uint24 fee, int24 tickSpacing, address hook
        ) {
        ...
        } catch { console.log("rareEthPoolKey: <error>"); }

        if (guard != address(0)) {
            try d.approvedHooks(guard) returns (bool value) {
                _emitBool("approvedHooks(guard)", value);
                _diagIf(!value, "FeeDistributor.approvedHooks(guard) == false: LiquidGuard cannot call notifyFee - fees will be silently skipped");
                _okIf(value, "FeeDistributor.approvedHooks(guard) == true");
            } catch { console.log("approvedHooks(guard): <error>"); }
        }
    }
    ...
    function _registry(address target, address factory, address guard) private {
        ...
        if (factory != address(0)) {
            try r.isWriter(factory) returns (bool value) {
                ...
                _diagIf(!value, "Registry.isWriter(factory) == false: Factory cannot register tokens or set beneficiaries");
                _okIf(value, "Registry.isWriter(factory) == true");
            } catch { console.log("isWriter(factory): <error>"); }
        }
        ...
        if (guard != address(0)) {
            ...
            } catch { console.log("isWriter(guard): <error>"); }
        }
    }
    ...
    function _migrationExec(address target, address guard, address router) private {
        ...
        try m.owner() returns (address value) { _emit("owner", value); }
        catch { console.log("owner: <error>"); }

        try m.protocolVault() returns (address value) { _emit("protocolVault", value); }
        catch { console.log("protocolVault: <error>"); }

        try m.liquidRegistry() returns (address value) { _emit("liquidRegistry", value); }
        catch { console.log("liquidRegistry: <error>"); }

        if (guard != address(0)) {
            try m.approvedHooks(guard) returns (bool value) {
                _emitBool("approvedHooks(guard)", value);
                _diagIf(!value, "MigrationExecutor.approvedHooks(guard) == false: migration to LiquidGuard hook will revert");
        ...
        if (router != address(0)) {
            ...
            } catch { console.log("approvedHooks(router): <error>"); }
        }
    ...
    function _token(address target) private view {
        ...
        } catch { console.log("token.poolKey: <error>"); }
    }
    ...
    function _derivedChecks(
        ...
        try IFactoryDiag(FACTORY).poolHooks() returns (address value) { factoryHooks = value; } catch {}
        try IGuardDiag(GUARD).factory() returns (address value) { guardFactory = value; } catch {}
        ...
        if (factoryHooks != address(0) && guardFactory != address(0)) {
            _diagIf(factoryHooks == GUARD && guardFactory != FACTORY,
        ...
        try IGuardDiag(GUARD).feeDistributor() returns (address value) { distViaGuard = value; } catch {}
        ...
        try IFactoryDiag(FACTORY).liquidRegistry() returns (address value) { factoryRegistry = value; } catch {}
        try IRouterDiag(ROUTER).liquidRegistry() returns (address value) { routerRegistry = value; } catch {}

        if (REGISTRY != address(0)) {
            bool factoryIsWriter;
            try IRegistryDiag(REGISTRY).isWriter(FACTORY) returns (bool v) { factoryIsWriter = v; } catch {}
        ...
        if (MIG_EXEC != address(0) && REGISTRY != address(0)) {
            address migRegistry;
            try IMigrationExecutorDiag(MIG_EXEC).liquidRegistry() returns (address value) { migRegistry = value; } catch {}
        ...
        if (MIG_EXEC != address(0)) {
            ...
            try IFactoryDiag(FACTORY).migrationExecutor() returns (address v) { factoryMigExec = v; } catch {}
            if (factoryMigExec != address(0)) {
                _diagIf(factoryMigExec != MIG_EXEC, "Factory.migrationExecutor != expected MIG_EXEC - tokens will check wrong executor");
                _okIf(factoryMigExec == MIG_EXEC, "Factory.migrationExecutor == MIG_EXEC");
            }
        }

        // --- Factory.protocolFeeRecipient vs FeeDistributor.PROTOCOL_FEE_RECIPIENT ---
        if (FEE_DIST != address(0)) {
            address factoryFeeRecip;
            address distFeeRecip;
            try IFactoryDiag(FACTORY).protocolFeeRecipient() returns (address v) { factoryFeeRecip = v; } catch {}
            try IFeeDistributorDiag(FEE_DIST).PROTOCOL_FEE_RECIPIENT() returns (address v) { distFeeRecip = v; } catch {}
            if (factoryFeeRecip != address(0) && distFeeRecip != address(0)) {
                _diagIf(factoryFeeRecip != distFeeRecip,
                    "Factory.protocolFeeRecipient != FeeDistributor.PROTOCOL_FEE_RECIPIENT - fee recipient mismatch");
                _okIf(factoryFeeRecip == distFeeRecip,
                    "Factory.protocolFeeRecipient == FeeDistributor.PROTOCOL_FEE_RECIPIENT");
            }
        }
        ...
        if (FEE_DIST != address(0) && REGISTRY != address(0)) {
            try IFeeDistributorDiag(FEE_DIST).beneficiaryRegistry() returns (address value) {
                _diagIf(value != REGISTRY, "FeeDistributor.beneficiaryRegistry != LiquidRegistry - beneficiary payouts will be wrong");
    ...
    function _contractIdentityChecks(
        ...
        if (FACTORY != address(0)) {
            try IFactoryDiag(FACTORY).poolManager() returns (address v) { canonicalPoolManager = v; } catch {}
            try IFactoryDiag(FACTORY).baseToken()   returns (address v) { canonicalBaseToken = v; }   catch {}
            try IFactoryDiag(FACTORY).liquidRegistry() returns (address v) { canonicalRegistry = v; } catch {}
        }
        ...
        if (GUARD != address(0)) {
            address guardPM;
            address guardRare;
            try IGuardDiag(GUARD).POOL_MANAGER() returns (address v) { guardPM = v; } catch {
                _diagIf(true, "GUARD: POOL_MANAGER() call failed - may not be a LiquidGuard contract");
            }
            try IGuardDiag(GUARD).RARE_TOKEN() returns (address v) { guardRare = v; } catch {
            ...
            if (canonicalPoolManager != address(0) && guardPM != address(0)) {
                _diagIf(guardPM != canonicalPoolManager, "GUARD.POOL_MANAGER != Factory.poolManager - wrong contract or misconfigured");
            ...
            if (canonicalBaseToken != address(0) && guardRare != address(0)) {
                _diagIf(guardRare != canonicalBaseToken, "GUARD.RARE_TOKEN != Factory.baseToken - wrong contract or misconfigured");
        ...
        if (FEE_DIST != address(0)) {
            address distPM;
            address distRare;
            try IFeeDistributorDiag(FEE_DIST).POOL_MANAGER() returns (address v) { distPM = v; } catch {
                _diagIf(true, "FEE_DIST: POOL_MANAGER() call failed - may not be a FeeDistributor contract");
            }
            try IFeeDistributorDiag(FEE_DIST).RARE_TOKEN() returns (address v) { distRare = v; } catch {
            ...
            if (canonicalPoolManager != address(0) && distPM != address(0)) {
                _diagIf(distPM != canonicalPoolManager, "FEE_DIST.POOL_MANAGER != Factory.poolManager - wrong contract or misconfigured");
            ...
            if (canonicalBaseToken != address(0) && distRare != address(0)) {
                _diagIf(distRare != canonicalBaseToken, "FEE_DIST.RARE_TOKEN != Factory.baseToken - wrong contract or misconfigured");
            ...
            if (GUARD != address(0)) {
                ...
                bool guardFeeOk;
                bool distFeeOk;
                try IGuardDiag(GUARD).totalFeeBPS()         returns (uint16 v) { guardFeeBPS = v; guardFeeOk = true; } catch {}
                try IFeeDistributorDiag(FEE_DIST).totalFeeBPS() returns (uint16 v) { distFeeBPS = v; distFeeOk = true; } catch {}
                if (guardFeeOk && distFeeOk) {
                    _diagIf(guardFeeBPS != distFeeBPS, "GUARD.totalFeeBPS != FEE_DIST.totalFeeBPS - fee split mismatch, Guard may be stale");
        ...
        if (ROUTER != address(0)) {
            address routerUR;
            try IRouterDiag(ROUTER).universalRouter() returns (address v) { routerUR = v; } catch {
            ...
            if (canonicalRegistry != address(0)) {
                address routerReg;
                try IRouterDiag(ROUTER).liquidRegistry() returns (address v) { routerReg = v; } catch {}
                if (routerReg != address(0)) {
                    _diagIf(routerReg != canonicalRegistry, "ROUTER: liquidRegistry() mismatch vs Factory - wrong contract or misconfigured");
        ...
        if (MIG_EXEC != address(0)) {
            address migVault;
            try IMigrationExecutorDiag(MIG_EXEC).protocolVault() returns (address v) { migVault = v; } catch {
            ...
            if (canonicalRegistry != address(0)) {
                address migReg;
                try IMigrationExecutorDiag(MIG_EXEC).liquidRegistry() returns (address v) { migReg = v; } catch {}
                if (migReg != address(0)) {
                    _diagIf(migReg != canonicalRegistry, "MIG_EXEC: liquidRegistry() mismatch vs Factory - wrong contract or misconfigured");
    ...
    function _tickRangeChecks(address FACTORY) private {
        ...
        try IFactoryDiag(FACTORY).lpTickLower()     returns (int24 v) { lower = v; lowerOk = true; }     catch {}
        try IFactoryDiag(FACTORY).lpTickUpper()      returns (int24 v) { upper = v; upperOk = true; }     catch {}
        try IFactoryDiag(FACTORY).poolTickSpacing()  returns (int24 v) { spacing = v; spacingOk = true; } catch {}

        if (!lowerOk || !upperOk || !spacingOk) {
            console.log("could not read tick config from Factory");
            return;
        }

        _diagIf(lower >= upper, "lpTickLower >= lpTickUpper - invalid tick range, pool creation will revert");
        _okIf(lower < upper, "lpTickLower < lpTickUpper");

        _diagIf(spacing <= 0, "poolTickSpacing <= 0 - invalid, pool creation will revert");
        if (spacing > 0) {
            bool lowerAligned = lower % spacing == 0;
            bool upperAligned = upper % spacing == 0;
            _diagIf(!lowerAligned, "lpTickLower is not a multiple of poolTickSpacing - pool creation will revert");
            _diagIf(!upperAligned, "lpTickUpper is not a multiple of poolTickSpacing - pool creation will revert");
            _okIf(lowerAligned && upperAligned, "tick bounds are aligned to poolTickSpacing");
        }
    }
    ...
    function _rareEthPoolKeyCheck(address FEE_DIST, address FACTORY, bytes32 knownPoolId) private {
        ...
        if (knownPoolId != bytes32(0)) {
            ...
            _diagIf(computedPoolId != knownPoolId,
                "rareEthPoolKey produces a different poolId than NetworkConfig.rareEthPoolId - pool key parameters are wrong");
            _okIf(computedPoolId == knownPoolId,
                "rareEthPoolKey poolId matches NetworkConfig.rareEthPoolId");
        }

        // Try to read slot0 from PoolManager to verify the pool exists on-chain
        address pm;
        if (FACTORY != address(0)) {
            try IFactoryDiag(FACTORY).poolManager() returns (address v) { pm = v; } catch {}
        }
        if (pm == address(0)) {
            try IFeeDistributorDiag(FEE_DIST).POOL_MANAGER() returns (address v) { pm = v; } catch {}
        }
        ...
        try IPoolManagerDiag(pm).getSlot0(computedPoolId) returns (uint160 sqrtPrice, int24, uint24, uint24) {
            if (sqrtPrice != 0) {
                ...
                _diagIf(true, "rareEthPoolKey pool sqrtPriceX96 == 0 - pool not initialized, conversion will fall back to RARE");
            }
        } catch {}
