...
import {LiquidRenderSimpleV4} from "liquid-editions/examples/LiquidRenderSimpleV4.sol";
import {ILiquid} from "liquid-editions/interfaces/ILiquid.sol";
import {console} from "forge-std/console.sol";
import {Script} from "forge-std/Script.sol";

/**
 * @title DeployLiquidRenderSimpleV4
 * @notice Script to deploy a LiquidRenderSimpleV4 render contract
 * @dev
 *
 * This script deploys a simple render contract that implements IRender interface
 * and provides dynamic tokenURI() for Liquid Edition contracts.
 * V4 emits NFT metadata as an HTML artifact with an embedded SVG preview.
 *
 ...
 * - COLLECTION_NAME: Name for the collection (default: "Liquid Lens Simple V4")
 * - COLLECTION_DESCRIPTION: Description for the collection (default: "Hyper-reactive generative collection where tiny market-state changes trigger major visual shifts with HTML metadata output")
 *
 * Usage:
 *   forge script script/DeployLiquidRenderSimpleV4.s.sol:DeployLiquidRenderSimpleV4 --rpc-url $RPC_URL --broadcast
 */
contract DeployLiquidRenderSimpleV4 is Script {
    function run() external {
        ...
        try vm.envString("COLLECTION_NAME") returns (string memory name) {
            ...
            collectionName = "Liquid Lens Simple V4";
        }
        ...
        try vm.envString("COLLECTION_DESCRIPTION") returns (
            ...
            collectionDescription = "Hyper-reactive generative collection where tiny market-state changes trigger major visual shifts with HTML metadata output";
        }

        address deployerAddress = vm.addr(deployerPrivateKey);

        console.log("=== DEPLOYING LIQUID RENDER SIMPLE V4 ===");
        console.log("");
        ...
        // Deploy the LiquidRenderSimpleV4 contract
        console.log("Deploying LiquidRenderSimpleV4 contract...");
        LiquidRenderSimpleV4 render = new LiquidRenderSimpleV4(
            liquidEditionAddress,
            collectionName,
            collectionDescription
        );

        console.log("LiquidRenderSimpleV4 deployed at:");
        ...
        if (deployerAddress == tokenCreator) {
            ...
            try liquidEdition.setRenderContract(address(render)) {
                ...
                console.log(
                    "Liquid Edition tokenURI() will now use the HTML render contract"
                );
        ...
        console.log("");
        console.log("LiquidRenderSimpleV4 Contract:");
        console.logAddress(address(render));
        console.log("");
        ...
        console.log("Render media:", "HTML metadata page with embedded SVG preview");
        console.log("");
