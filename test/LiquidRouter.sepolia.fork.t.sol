...
contract LiquidRouterSepoliaForkTest is Test {
    ...
    function testFork_BuyRareWithEth() public {
        ...
        uint256 ethForSwap = (ethAmount * 9600) / 10000; // 96% after 4% fee
        ...
        assertEq(
            rareBalanceAfter - rareBalanceBefore,
            tokensReceived,
            "Balance should match received"
        );
        assertEq(
            ethBalanceBefore - ethBalanceAfter,
            ethAmount,
            "Should spend exact ETH amount"
        );
    ...
    function testFork_SellRareForEth() public {
        ...
        uint256 ethForSwap = (ethToBuy * 9600) / 10000; // 96% after 4% fee
        ...
        assertEq(
            rareBalanceBefore - rareBalanceAfter,
            tokensToSell,
            "Should sell exact token amount"
        );
        assertEq(
            ethBalanceAfter - ethBalanceBefore,
            ethReceived,
            "ETH balance should increase by received amount"
        );
    ...
    function testFork_BuySellRoundTrip() public {
        ...
        assertEq(
            finalRare,
            initialRare,
            "RARE balance should be unchanged after round trip"
        );
    ...
    function _encodeBuyRoute(
        ...
        bytes memory path = abi.encodePacked(WETH, uint24(3000), RARE_TOKEN);
    ...
    function _encodeSellRoute(
        ...
        bytes memory path = abi.encodePacked(RARE_TOKEN, uint24(3000), WETH);
