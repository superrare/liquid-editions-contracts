...
/**
 * @title SepoliaBehaviorBase
 * @notice Shared base for Sepolia user-behavior scenario tests.
 *         Assumes the Sepolia RARE/ETH pool is already synced to mainnet price
 *         (via SyncSepoliaRareEthPrice script). Handles price caching, route
 *         encoding, fee parsing, trade execution, and console formatting.
 *
 * @dev Subclasses must:
 *   1. Call _setupSepoliaBehavior() in setUp().
 *   2. Set `token` to the deployed ILiquid instance.
 *   3. Set `_rareIsCurrency0` from token.poolKey().
 */
abstract contract SepoliaBehaviorBase is Test {
    ...
    // ETH/RARE V4 pool parameters (same on Sepolia as mainnet)
    ...
    // RARE/ETH price cached from the live Sepolia V4 pool
    ...
    /**
     * @notice Core setup shared by all Sepolia behavior tests.
     *         Selects the Sepolia fork, binds deployed contracts, and caches
     *         the current RARE/ETH price from the already-synced pool.
     */
    function _setupSepoliaBehavior() internal {
        vm.createSelectFork("sepolia");

        config  = NetworkConfig.getConfig(block.chainid); // 11155111
    ...
    function _printRareEthPrice() internal view {
        ...
        console.log("--- RARE/ETH PRICE (Sepolia V4 pool) ---");
