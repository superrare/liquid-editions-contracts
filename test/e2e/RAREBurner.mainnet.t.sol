...
contract RAREBurnerForkTest is Test {
    ...
    function testFlushHappyPath_OnBaseSepoliaFork() public {
        ...
        vm.prank(user1);
        testBurner.depositForBurn{value: 0.01 ether}();

        uint256 pendingBefore = testBurner.pendingEth();
        testBurner.flush();
        uint256 pendingAfter = testBurner.pendingEth();

        // Best-effort: flush either succeeds (pending decremented) or fails gracefully (unchanged)
        assertTrue(
            pendingAfter <= pendingBefore,
            "pendingEth should not increase after flush"
        );
    }
...
contract RARETokenBurnMechanismTest is Test {
    ...
    function testRARERejectsTransferToZeroAddress() public {
        ...
        for (uint256 i = 0; i < potentialHolders.length; i++) {
            (bool balSuccess, bytes memory balData) = config
                .rareToken
                .staticcall(
        ...
        if (holder == address(0)) {
            ...
            (bool checkSuccess, bytes memory checkData) = config
                .rareToken
                .staticcall(
        ...
        (bool zeroBalSuccess, bytes memory zeroBalData) = config
            .rareToken
            .staticcall(
        ...
        vm.prank(holder);
        (bool transferSuccess, bytes memory transferData) = config
            .rareToken
            .call(
                abi.encodeWithSignature(
        ...
        if (!transferSuccess) {
            ...
            // Check new balances
            (bool newBalSuccess, bytes memory newBalData) = config
                .rareToken
                .staticcall(
                    abi.encodeWithSignature("balanceOf(address)", holder)
                );
            ...
            (bool newZeroSuccess, bytes memory newZeroData) = config
                .rareToken
                .staticcall(
                    abi.encodeWithSignature("balanceOf(address)", address(0))
                );
