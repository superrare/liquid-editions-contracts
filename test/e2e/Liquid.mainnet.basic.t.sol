...
contract LiquidInstantMainnetBasicTest is Test, InitGuardTestHelper {
    ...
    function testInitializationWithMinimumRareOnlyProvidesLiquidity() public {
        ...
        address newTokenAddress = factory.createLiquidTokenMultiCurve(
            creator, "ipfs://test-token-uri-2", "MIN_LIQUID", "ML", MIN_RARE_AMOUNT, _defaultSingleCurve(factory)
        );
        vm.stopPrank();

        LiquidMultiCurve newLiquidInstant = LiquidMultiCurve(payable(newTokenAddress));

        // Verify RARE was spent
        assertEq(mockRARE.balanceOf(creator), creatorInitialRareBalance - MIN_RARE_AMOUNT);
    ...
    function testReceiveFunction() public {
        ...
        assertFalse(success, "Call should revert - LiquidMultiCurve has no receive() function");

        // Verify user's ETH balance is unchanged (transaction reverted)
        assertEq(user1.balance, INITIAL_BALANCE, "User ETH should be unchanged after reverted call");
    ...
    function testPoolLiquidityDeployment() public {
        ...
        address newTokenAddress = factory.createLiquidTokenMultiCurve(
            creator, "ipfs://liquidity-test", "LIQUIDITY_TEST", "LT", LIQUIDITY_RARE, _defaultSingleCurve(factory)
        );
        vm.stopPrank();
        ...
        assertTrue(PoolId.unwrap(poolId) != bytes32(0), "Pool should be created");
        ...
        assertTrue(positions > 0 || liquidity > 0, "LP position should have liquidity greater than 0");
    ...
    function testFactoryRARELiquidityCreation() public {
        ...
        LiquidFactory testFactory = new LiquidFactory(admin, config.uniswapV4PoolManager, testInitGuardAddr, 60);
        ...
        testFactory.setLiquidMultiCurveImplementation(address(factoryLiquidInstantImplementation));
        ...
        address tokenAddress = testFactory.createLiquidTokenMultiCurve(
            creator, "ipfs://factory-test", "FACTORY_TEST", "FT", FACTORY_ETH_AMOUNT, _defaultSingleCurve(testFactory)
        );
        vm.stopPrank();

        // Verify RARE was spent (all RARE goes to liquidity)
        LiquidMultiCurve factoryLiquidInstant = LiquidMultiCurve(payable(tokenAddress));
        assertEq(
        ...
        // Verify pool was created
        assertTrue(PoolId.unwrap(factoryLiquidInstant.poolId()) != bytes32(0), "Factory should create pool");
        ...
        (uint160 sqrtPriceX96,,,) = pm.getSlot0(poolId);
        assertTrue(sqrtPriceX96 > 0, "Factory-created pool should be initialized");
        ...
        assertTrue(positions > 0 || liquidity > 0, "FACTORY SHOULD CREATE LIQUIDITY IN POOL");
    ...
    function testPoolLiquidityDeploymentMinimumAmount() public {
        ...
        // Verify pool was created
        assertTrue(
            PoolId.unwrap(newLiquidInstant.poolId()) != bytes32(0), "Pool should be created even with minimum RARE"
        );
        ...
        PoolId poolId = newLiquidInstant.poolId();
        (uint160 sqrtPriceX96,,,) = pm.getSlot0(poolId);
        assertTrue(sqrtPriceX96 > 0, "Pool should be initialized with minimum RARE");
        ...
        assertTrue(positions > 0 || liquidity > 0, "LP position should have liquidity > 0 even with minimum RARE");
    ...
    function testPoolInitializationWithLiquidInstantAsCurrency0() public {
        ...
        bytes32 salt = keccak256("testPoolInitializationWithLiquidInstantAsCurrency0");
        ...
        LiquidFactory testFactory = new LiquidFactory(admin, config.uniswapV4PoolManager, testInitGuardAddr2, 60);
        ...
        if (actualOrdering) {
            ...
            for (uint256 i = 0; i < 500; i++) {
                bytes32 trySalt = keccak256(abi.encodePacked("highAddress", i, liquidAddress));
                ...
                assembly {
                    ...
                    computedAddress := and(hash, 0xffffffffffffffffffffffffffffffffffffffff)
                }
            ...
            if (highRARE != address(0) && uint160(highRARE) > uint160(liquidAddress)) {
                ...
                testFactory = new LiquidFactory(admin, config.uniswapV4PoolManager, testInitGuardAddr3, 60);
        ...
        assertTrue(PoolId.unwrap(poolId) != bytes32(0), "Pool should be initialized");
        ...
        assertGe(currentTick, tickLower, "Price should be within lower bound");
        assertLe(currentTick, tickUpper, "Price should be within upper bound");
        ...
        (uint256 rarePerToken, uint256 tokenPerRare) = testLiquidInstant.getCurrentPrice();
        // When LIQUID is currency0, rarePerToken should be low (cheap LIQUID)
        assertTrue(rarePerToken < 1e18, "LIQUID should start cheap (low RARE per token)");
        ...
        assertEq(creatorBalance, CREATOR_LAUNCH_REWARD, "Creator should receive launch reward");

        // Verify total supply
        assertEq(testLiquidInstant.totalSupply(), 1_000_000e18, "Total supply should be MAX_TOTAL_SUPPLY");
        ...
        uint256 contractBalance = testLiquidInstant.balanceOf(address(testLiquidInstant));
        ...
        assertTrue(contractBalance < POOL_LAUNCH_SUPPLY / 10, "Most tokens should be in pool, not contract");
