...
///         Plotter art-inspired generative SVG artwork visualizing Liquid Edition market state.
///         Features flow fields, hatching, and line-only aesthetics.
/// @dev Not an ERC721 contract - just implements IRender interface for tokenURI() passthrough.
...
contract LiquidRenderSimple is IRender {
    ...
    /// @notice Returns the token URI (no parameters) - main function for Liquid contract
    /// @dev Reads market state from Liquid Edition and generates dynamic SVG artwork
    /// @return The token URI string (JSON metadata with base64-encoded SVG)
    function tokenURI() external view override returns (string memory) {
        ...
        // Use a fixed seed based on contract address for consistent styling
        uint256 masterSeed = uint256(
            keccak256(
                abi.encodePacked(
                    address(this),
                    rarePerToken,
        ...
        string memory paletteName = _getPaletteName(masterSeed);
        return _generateMetadataJSON(paletteName, metrics, svg);
    }

    /// @notice Returns the token URI (ERC721-style compatibility)
    ...
    function _generateSVG(
        ...
        string[4] memory palette = _getPalette(masterSeed);
    ...
    function _generateMetadataJSON(
        ...
        string memory attributes = _buildAttributes(paletteName, metrics);
