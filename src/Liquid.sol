    ...
    IUnlockCallback
{
    ...
    function _unlockInitializePool(
        ...
        // Both currencies are ERC20 tokens now (RARE and LIQUID)
        if (delta0 < 0) {
            uint128 owed0 = _toUint128Neg(delta0);
            // currency0 is either RARE or LIQUID depending on address ordering
            address token0 = Currency.unwrap(poolKey.currency0);
            pm.sync(poolKey.currency0);
            IERC20(token0).transfer(address(pm), owed0);
            pm.settle();
        }

        if (delta1 < 0) {
            uint128 owed1 = _toUint128Neg(delta1);
            // currency1 is either RARE or LIQUID depending on address ordering
            address token1 = Currency.unwrap(poolKey.currency1);
            pm.sync(poolKey.currency1);
            if (token1 == address(this)) {
                // LIQUID token - use internal transfer
                _transfer(address(this), address(pm), owed1);
            } else {
                // RARE token - use ERC20 transfer
                IERC20(token1).transfer(address(pm), owed1);
            }
            pm.settle();
        }
        ...
        if (remainingRare > 1) {
            ...
            IERC20(baseToken).transfer(tokenCreator, remainingRare);
        }
